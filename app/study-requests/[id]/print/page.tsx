...
import type { StudyRequestRow } from '@/utils/clinicalTypes'
...
interface SnapshotStudy { section: string; name: string; description?: string | null; indication?: string | null }

export default async function PrintStudyRequestPage({ params, searchParams }: PageProps) {
  ...
  const access = await loadPublicDocument<StudyRequestRow>('study_requests', id, code)
  if (access.status === 'notfound') return notFound()
  if (access.status === 'gate') {
    return <DocumentCodeGate basePath={`/study-requests/${id}/print`} hasError={!!access.hasError} docLabel="Solicitud de Estudios" />
  }
  ...
  // Agrupar los estudios solicitados por sección conservando el orden.
  const studies: SnapshotStudy[] = Array.isArray(order.studies) ? order.studies : []
  const grouped: { section: string; names: string[] }[] = []
  for (const s of studies) {
    let g = grouped.find((x) => x.section === s.section)
    if (!g) { g = { section: s.section, names: [] }; grouped.push(g) }
    g.names.push(s.name)
  }
  // Indicaciones por estudio (solo los que la tengan).
  const withIndication = studies.filter((s) => (s.indication || '').trim())
  const otherLines = (order.other_studies || '').split('\n').map((s: string) => s.trim()).filter(Boolean)

  // QR -> página pública de verificación de la solicitud (igual que las órdenes de laboratorio)
    ...
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        * { box-sizing: border-box; }
        ...
        /* Cuerpo: estudios solicitados por sección */
        ...
        /* Indicaciones para el paciente (una por estudio) */
        .indications { margin-top: 14px; flex: 1; }
        .ind { break-inside: avoid; page-break-inside: avoid; margin-bottom: 9px; padding: 8px 11px; background: #f8fafc; border: 1px solid #e9eef4; border-radius: 8px; }
        .ind-name { font-size: 11px; font-weight: 800; color: #1e3a5f; margin: 0 0 3px; }
        .ind-body { font-size: 9.5px; color: #0f172a; line-height: 1.5; white-space: pre-line; margin: 0; }
      ...
      <div className="page-wrap">
        <div className="sheet">
          <div className="title-box"><span>SOLICITUD DE ESTUDIOS</span></div>
          ...
          <div className="body">
            <p className="body-title">Estudios Solicitados</p>
            {grouped.length === 0 && otherLines.length === 0 ? (
              <p className="empty">No se especificaron estudios.</p>
            ) : (
              <div className="cats">
                {grouped.map((g) => (
                  <div key={g.section} className="cat">
                    <div className="cat-name">{g.section}</div>
                    {g.names.map((n, i) => (
          ...
          {/* Indicaciones para el paciente (una sección por estudio que la tenga) */}
          {withIndication.length > 0 && (
            <div className="indications">
              <p className="body-title">Indicaciones para el Paciente</p>
              {withIndication.map((s, i) => (
                <div key={i} className="ind">
                  <p className="ind-name">{s.name}</p>
                  <p className="ind-body">{(s.indication || '').trim()}</p>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="footer" style={qrDataUrl ? undefined : { justifyContent: 'flex-end' }}>
            {qrDataUrl && (
              <div className="qr-box">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={qrDataUrl} alt="Código QR de verificación de la solicitud" />
                <div>
                  <p className="qr-cap-title">Solicitud verificable</p>
                  <p className="qr-cap-text">Escanea el código para ver y validar esta solicitud en su versión digital.</p>
