-- Detalle de cada KPI del reporte (al hacer click en una tarjeta). Solo estadística/listado,
-- sin datos clínicos (sin diagnóstico/tratamiento/notas). SECURITY DEFINER + acotado a la clínica
-- del usuario. p_tipo: 'consultas' | 'citas' | 'no_show' | 'pacientes_nuevos'. Máx 500 filas recientes.
...
begin
  ...
  if p_tipo = 'consultas' then
    return coalesce((
      select jsonb_agg(jsonb_build_object('fecha', fecha, 'paciente', paciente, 'medico', medico) order by fecha desc)
      from (
    ...
    return coalesce((
      select jsonb_agg(jsonb_build_object('fecha', fecha, 'paciente', paciente, 'medico', medico, 'estado', estado) order by fecha desc)
      from (
